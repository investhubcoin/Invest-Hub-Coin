...
#if defined(NDEBUG)
#error "IHC cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 12 * 60 * 60;//OFF
unsigned int nStakeMinAge2 = 12 * 60 * 60;//OFF
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge)
{
    ...
    if (IsSporkActive(SPORK_21_STAKE_REQ_AG) && nTxTime >= GetSporkValue(SPORK_21_STAKE_REQ_AG)) {
    ...
    BOOST_FOREACH (const CTxIn& txin, tx.vin) {
        ...
        if (prevblock.nTime + nStakeMinAgeCurrent > nTxTime)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.IsCoinStake())
        return state.DoS(fDebug ? 100 : 0, error("AcceptToMemoryPool: coinstake as individual tx"),
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zIHC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zIHC spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zIHC spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 500000 && nHeight > 50000) {
        nSubsidy = 50 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from IHC
...
{
    RenameThread("ihc-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZIHCMinted()
{
...
void RecalculateZIHCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zIHC supply
...
bool RecalculateIHCSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zIHC with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zIHCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert IHC to zIHC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (IsSporkActive(SPORK_22_STAKE_REQ_SZ) && block.GetBlockTime() >= GetSporkValue(SPORK_22_STAKE_REQ_SZ)) {
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zIHC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zIHC serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          if (nHeight > DON_BLOCK) {
            ...
            if (block.vtx[1].vout[s - 2].scriptPubKey != EmFundScriptPubKey())
                return state.DoS(100, error("CheckBlock() : incorrect D fund payment"));
          }
          ...
          // Inputs
          std::vector<CTxIn> ihcInputs;
          std::vector<CTxIn> zIHCInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zIHCInputs.push_back(stakeIn);
              }else{
                  ihcInputs.push_back(stakeIn);
              }
          }
          const bool hasIHCInputs = !ihcInputs.empty();
          const bool hasZIHCInputs = !zIHCInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasIHCInputs)
                      ...
                      for (CTxIn ihcIn : ihcInputs){
                          if(ihcIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zIHC inputs.
              if(hasZIHCInputs){
                  for (CTxIn zPawsInput : zIHCInputs) {
                      CoinSpend spend = TxInToZerocoinSpend(zPawsInput);
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zihc");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zIHC mints and %d zIHC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // IHC: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    ...
    // SPORK_14 is used for 70002. Leave it 'ON' so they don't see 70001 nodes. They won't react to SPORK_15
    // messages because it's not in their code

    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 is used for 70001. Nodes with 70002/70003 don't see it and still get their protocol version via SPORK_14 and their
...
CScript EmFundScriptPubKey()
{
	return CScript() << ParseHex("0458d87715a5adeee3233dda13675b2714e6bc11f9e89401f1acc7e42a692bbe0e82cac64b4b683628a6b93702d06f2b3c3cf4334b2abb5447a89fa52a1d1baba7") << OP_CHECKSIG;
}

CScript OpFundScriptPubKey()
{
	return CScript() << ParseHex("046b2aae13ea37630a8bca9adfe956434d9bfda42a786d1d7219cefb2e1138bf3200ed5eda0e5ca61ff060439a1591f2c1a35eca1ba508844021ca524aa1a318c3") << OP_CHECKSIG;
}
