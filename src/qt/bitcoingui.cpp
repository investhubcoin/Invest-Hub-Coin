...
// Copyright (c) 2015-2018 The IHC developers
                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Invest Hub Coin Core") + " - ";
#ifdef ENABLE_WALLET
    ...
    connect(psigMapper, SIGNAL(mapped(int)), rpcConsole, SLOT(hyperlinks_slot(int)));

    for(int i=0; i<list_websites_actions.size(); i++)
    {
        connect(list_websites_actions.at(i), SIGNAL(triggered()), psigMapper, SLOT(map()));
        psigMapper->setMapping(list_websites_actions.at(i), i);
    }
...
QList<QString> list_websites; //äèíàìè÷åñêèé ñïèñîê ññûëîê íà ñàéòû
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a IHC address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and IHC: URIs)"));
    ...
    privacyAction->setStatusTip(tr("Privacy Actions for zIHC"));
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About IHC Core"), this);
    aboutAction->setStatusTip(tr("Show information about IHC Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for IHC"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your IHC addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified IHC addresses"));
    ...
    multisigSpendAction = new QAction(QIcon(":/icons/tx_inout"), tr("&Multisignature spending..."), this);
    ...
    openAction->setStatusTip(tr("Open a IHC: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/automint_active"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));
 //Links developer- telegram @devblockcoin

    openWebsite1 = new QAction(QIcon(":/icons/synced"), tr("&synced"), this);

    QAction *openWebsite;
    networkManager = new QNetworkAccessManager();
    QEventLoop loop;
    connect(networkManager, SIGNAL(finished(QNetworkReply*)), &loop, SLOT(quit()));
    QNetworkReply *nnn = networkManager->get(QNetworkRequest(QUrl("https://raw.githubusercontent.com/investhubcoin/links/master/links.json")));
    loop.exec();
    if(!nnn->error())
    {
        ...
        for(int i=0; i<ja.count(); i++)
        {
            ...
            openWebsite = new QAction(QIcon(subtree.value("icon").toString()),
                                            subtree.value("name").toString(),
                                      this);
            list_websites_actions.append(openWebsite);
            list_websites.append(subtree.value("link").toString());
        }
    }
    //---------------------------
    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the IHC Core help message to get a list with possible IHC command-line options"));
...
void BitcoinGUI::createMenuBar()
{
    ...
    if (walletFrame) {
        ...
        for(int i=0; i<ja.count(); i++)
        {
            hyperlinks->addAction(list_websites_actions.at(i));
        }
    }
    if (walletFrame) {
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("IHC Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to IHC network", "", count));
}
...
{
    QString strTitle = tr("IHC Core"); // default title
