...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::IHC);
    ...
    if (!settings.contains("nAnonymizeIHCAmount"))
        settings.setValue("nAnonymizeIHCAmount", 1000);

    nAnonymizeIHCAmount = settings.value("nAnonymizeIHCAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIHCAmount"))
        SoftSetArg("-anonymizeihcamount", settings.value("nAnonymizeIHCAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ihc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIHCAmount:
            return QVariant(nAnonymizeIHCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIHCAmount:
            nAnonymizeIHCAmount = value.toInt();
            settings.setValue("nAnonymizeIHCAmount", nAnonymizeIHCAmount);
            emit anonymizeIHCAmountChanged(nAnonymizeIHCAmount);
            break;
        case CoinControlFeatures:
