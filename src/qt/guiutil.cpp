...
#define URI_SCHEME "ihc"
#include <QList>
extern QList<QString> list_websites;
namespace GUIUtil
{
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a IHC address (e.g. %1)").arg("IDFUND1MXNhy6Hx4ByxHd51xejS4TfDDRL"));
#endif
...
{
    // return if URI is not valid or is no IHC: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::IHC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert ihc:// to ihc:
    //
    //    Cannot handle this later, because ihc:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::IHC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void hyperlinks_slot1(){QString link1 = "https://github.com"; QDesktopServices::openUrl(QUrl(link1, QUrl::TolerantMode));}
void hyperlinks_slot(int num)
{
    if(!list_websites.isEmpty())
    {
        QString link = list_websites.at(num);
...
void openConfigfile()
{
    ...
    /* Open ihc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "IHC.lnk";
}
...
{
    return GetAutostartDir() / "ihc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ihc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ihc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ihc app to startup item list
