...
#include "ui_zihccontroldialog.h"
...
std::list<std::string> ZIHCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZIHCControlDialog::listMints;

ZIHCControlDialog::ZIHCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZIHCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZIHCControlDialog::~ZIHCControlDialog()
{
    delete ui;
}

void ZIHCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIHCControlDialog::updateList()
{
...
void ZIHCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIHCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZIHC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIHCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZIHCControlDialog::GetSelectedMints()
{
...
void ZIHCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
