...
// IHC: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_19_LOCK_OBFS) r = SPORK_19_LOCK_OBFS_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_19_LOCK_OBFS") return SPORK_19_LOCK_OBFS;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_19_LOCK_OBFS) return "SPORK_19_LOCK_OBFS";
