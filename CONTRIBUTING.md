Contributing to IHC Core
============================

The IHC Core project operates an open contributor model where anyone is
...
The following applies to code changes to the IHC Core project, and is not to be
confused with overall IHC Network Protocol consensus changes.

Whether a pull request is merged into IHC Core rests with the project merge
...
Patches that change IHC consensus rules are considerably more involved than
...
Where a patch set proposes to change the IHC consensus, it must have been
...
The project leader is the release manager for each IHC Core release.
